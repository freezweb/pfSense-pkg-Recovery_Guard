...
final class SupervisorBusy extends \RuntimeException {}
...
final class ServiceLoop
{
    ...
    {
        $this->exclusive(function () use ($cycle, $stopping, $report): void {
            $previous = null;
            while (!$stopping()) {
                $start = hrtime(true);
                $result = $cycle();
    ...
    /** The reboot worker must own this same lease after the monitor exits. */
    public function exclusive(\Closure $operation): mixed
    {
        if ($this->lease !== null) throw new \RuntimeException('Nested supervisor lease');
        ...
        try {
            if (!flock($this->lease, LOCK_EX | LOCK_NB)) throw new SupervisorBusy('Supervisor already running');
