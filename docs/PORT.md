...
The menu points to `services_recovery_guard.php`, with the native page privilege declaration and `guiconfig.inc` authentication/CSRF path. Native CE 2.9.0 HTTPS login, CSRF rejection, authenticated settings save and diagnostics export pass. Eight native restricted-user privilege checks pass; visual browser rendering remains required. The package ships a runtime privilege definition under pkg/priv because page metadata comments alone are not registered. Synthetic adapter tests do not establish these properties.
...
Use the actual target's PHP version when preparing its native builder. The inspected pfSense development system port excludes PHP 8.3 and 8.4, despite the generic ports framework's 8.4 default. The CE 2.9.0 laboratory build passes with DEFAULT_VERSIONS=php=8.5, the actual kern.osreldate and a private FreeBSD 16 compiler sysroot. Native development-channel testing remains separate from this stable 2.9 build.

The service XML deliberately leaves rcfile empty and uses native startcmd/stopcmd hooks. pfSense removes a registered rcfile before custom deinstallation; setting it to this package-owned script caused a fatal second stop and a missing-file warning. pkg-plist retains ownership until normal removal. The starts_on_sync marker preserves pfSense's after-sync behavior. See the guarded native lifecycle regression test in LAB.md.
