...
These limits assume a functioning kernel and process scheduler. `proc_close`, process creation and kernel I/O cannot provide a hardware-level guarantee during a kernel freeze. The timeout paths and descendant behavior still require the isolated lab suite. Do not represent this adapter as a verified repair receipt.
...
- `tests/process-lab.php` refuses execution outside FreeBSD and without the explicit lab environment marker. Its timeout/output/descendant cases are prepared but **not yet executed**. The marker is an operator guard, not an isolation mechanism.
