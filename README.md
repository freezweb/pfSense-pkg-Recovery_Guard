...
**Development status:** tested libraries, native configuration editor, monitor daemon and a stageable development port. Automatic repair and reboot execution remain disabled. An isolated official CE 2.9 guest now passes native package lifecycle and authenticated GUI checks, real PHP-FPM fault/repair and a native cleanup reboot harness. The full autonomous fault sequence and supported-version matrix remain release work. Official pfSense repository inclusion remains an uncompleted goal.
...
- Native monitor service: separate PHP CLI process, exclusive supervisor lease, interruptible scheduling, bounded log-worker requests and signal-driven shutdown. First initialization creates the durable journal; normal lifecycle operations preserve it and refuse to reset missing or invalid budgets. Nineteen native CE 2.9 package lifecycle checks pass, including exact retained budgets and privilege registration.
- Repair controller and executor: native FreeBSD process ownership preserves daemons after a successful controller exit and verifies descendant cleanup on failure/timeout. A PHP-FPM challenge distinguishes service health from script exit status. Seventeen isolated lab checks include a real private PHP-FPM instance through the integrated action executor and upgrade lease. The daemon is wired to the adapters behind its monitor-only release gate; a real CE 2.9 service fault now passes the original native repair path after a real two-minute confirmation interval, with a private test ledger.
- DiagnosticJournal: durable, bounded action evidence with 64-record retention, explicit outcomes, sanitized fields and conservative failure handling. Monitor proposals are recorded as inhibited. The native page includes a history and JSON export; native authenticated settings/export and restricted-user privilege checks pass; visual browser review remains open.
- Reboot handoff: durable one-use intent tied to the reserved budget, diagnostic evidence, boot and configuration. A worker takes the supervisor lease after retirement and rechecks maintenance/HA/upgrade gates and current PHP/LAN failure around the durable claim. Wall and monotonic deadlines prevent late execution. The native worker and dispatcher are wired behind the disabled active-mode release gate.
- Native action integration: fresh configuration/boot/health checks precede repair, late inhibition does not acknowledge a repair, and dispatch retires the supervisor after closing its workers. A shared native upgrade lease cooperates with the regular pfSense-upgrade wrapper and detects replaced lock inodes. It is not a universal mutex for direct administrator commands or the final native shutdown cleanup. See [native coordination limits](docs/PLATFORM.md).
...
- Reboot reconciliation: startup can record a new boot after a durably claimed request, using changed native boot identity, a reset monotonic clock, lower uptime and retained reservation/evidence. It preserves the original outcome, never replays an intent and never changes the action budget. A new boot does not prove its cause or service recovery. See [observation criteria](docs/REBOOT-OUTCOMES.md).
...
php tests/reconciliation.php
php tests/native-executor.php
...
On Windows, only directory-fsync is simulated. The journal tests have also run on pfSense CE 2.8.1 / PHP 8.3.19 with native file and directory fsync. Actual CE 2.9 PHP-FPM service recovery, package lifecycle and a native cleanup reboot harness now pass; the full autonomous reboot sequence and development-version matrix remain unproven.
...
Remaining work includes the full autonomous fault/reboot sequence, native notification integration, complete interlock/failure coverage, visual GUI review and the supported/current development-version matrix. See [acceptance tracking](docs/ACCEPTANCE.md).
